...
map_form = folium.Map(location=[nom.geocode(location).latitude, nom.geocode(
    location).longitude], zoom_start=20, tiles='Stamen Terrain')
...
folium.GeoJson(
    data=(open('world.json', 'r', encoding='utf-8-sig').read()), name="Border").add_to(map_form)
folium.LayerControl().add_to(map_form)
